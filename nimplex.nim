...
proc simplex_graph3_nim(
    ...
    func neighborsLink(i:int, x:Tensor): seq[int] =
        if x[0] != 0:
            result.add(i-binom(1+12-x[0], 1))
            result.add(i-1-binom(1+12-x[0], 1) )
        if x[1] != 0:
            result.add(i-1)
            result.add(i-1+binom(1+12-x[0], 1))
        if x[2] != 0:
...
proc simplex_graph4_nim(
    ...
    func neighborsLink(i:int, x:Tensor): seq[int] =
        ...
        if x[0] != 0:
            result.add(i - jump2 - jump1)
            result.add(i-1-jump2 - jump1)
            result.add(i - jump2)

        if x[1] != 0:
            result.add(i-jump1)             # ternary
            result.add(i-1-jump1)           # ternary
            result.add(i+jump2-jump1-x[1])  # quaternary

        if x[2] != 0:
            result.add(i-1)                 # binary
            result.add(i-1+jump1)           # ternary
            result.add(i-1+jump2-x[1])      # quaternary
        if x[3] != 0:
            result.add(i+1)                 # binary
            result.add(i+jump1)             # ternary
